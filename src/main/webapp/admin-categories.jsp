...
<body>
    ...
    <div class="main-content">
        ...
        <a href="${pageContext.request.contextPath}/admin/categories/add" class="add-button">Thêm Danh mục mới</a>

        <div class="sort-controls">
            <form action="${pageContext.request.contextPath}/admin/categories" method="get" style="display: inline;">
            ...
            <tbody>
                <c:forEach var="category" items="${requestScope.categoryList}">
                    <tr>
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${category.active}"><span class="status-active">Hoạt động</span></c:when>
                                <c:otherwise><span class="status-inactive">Không hoạt động</span></c:otherwise>
                            </c:choose>
                        </td>
                        <td class="action-buttons">
                            <a href="${pageContext.request.contextPath}/admin/categories/edit?id=${category.categoryID}" class="edit-btn">Sửa</a>
                            <a href="${pageContext.request.contextPath}/admin/categories/delete?id=${category.categoryID}" class="delete-btn" onclick="return confirm('Bạn có chắc chắn muốn xóa danh mục này không?');">Xóa</a>
                        </td>
                    </tr>
                </c:forEach>
                <c:if test="${empty requestScope.categoryList}">
                    <tr>
                        <td colspan="6">Không có danh mục nào để hiển thị.</td>
                    </tr>
                </c:if>
            </tbody>
        </table>
